...
void List_Dispose( List *list ) {
	...
	while(list->firstElement != NULL){
		...
		list->firstElement = new->nextElement;
		free(new);
	}
	list->activeElement = NULL;
	list->firstElement = NULL;
...
void List_InsertFirst( List *list, int data ) {
	...
	if(!(new = (ListElementPtr) malloc(sizeof(struct ListElement)))){
		List_Error();
		return;
	}
...
void List_DeleteAfter( List *list ) {
	...
	if(list->activeElement != NULL && list->firstElement != NULL){//if the list is not empty 
...
void List_InsertAfter( List *list, int data ) {
	...
	if(list->activeElement != NULL && list->firstElement != NULL){// if the list is not empty execute commnad, otherwise do nothing 
		ListElementPtr new;
...
void List_GetValue( List *list, int *dataPtr ) {
	if(list->activeElement != NULL){// if active node exist return its value, otherwise return error
